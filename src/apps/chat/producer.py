...
class Producer:
    ...
    @staticmethod
    def create_stream_data(chat_id: uuid.UUID, message: Message, user_id: uuid.UUID, username: str):
        """ Создаем сообщения для отправки в канал """
        stream_data = {
            str(chat_id): str({
                'id': str(message.id),
                'msg': str(message.msg),
                'user__id': str(user_id),
                'user__username': str(username),
