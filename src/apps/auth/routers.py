...
from src.apps.auth.schemas import Token, Msg, EmailRecover, ResetPassword
from src.apps.auth.send_email import send_reset_password_email
from src.apps.auth.services import registration_user, verify_registration_user, authenticate_user, create_access_token, \
    generate_password_reset_token, verify_password_reset_token, google_auth
from src.apps.user.schemas import UserIn, GoogleUserCreate
...
async def recover_password(email: EmailRecover, task: BackgroundTasks):
    """ Password Recovery """
    user = await UserService.get(email=email.email)
    if not user:
        raise HTTPException(
            status_code=404,
